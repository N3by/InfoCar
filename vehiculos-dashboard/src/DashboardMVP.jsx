...
export default function DashboardMVP() {
  ...
  // Estados para guardar valores de consulta exitosa
  const [consultaCedula, setConsultaCedula] = useState('');
  const [consultaPlaca, setConsultaPlaca] = useState('');

  // Sistema de limitación manual de toasts
  const toastQueue = useRef([]);
  const MAX_TOASTS = 3;

  const showLimitedToast = (message, type = 'error', duration = 4000) => {
    // Si ya hay 3 toasts, eliminar el más antiguo
    if (toastQueue.current.length >= MAX_TOASTS) {
      const oldestToast = toastQueue.current.shift();
      toast.dismiss(oldestToast);
    }

    // Agregar nuevo toast a la cola
    const toastId = type === 'error' ? toast.error(message, { duration }) : toast.success(message, { duration });
    toastQueue.current.push(toastId);

    // Limpiar la cola cuando el toast expire
    setTimeout(() => {
      const index = toastQueue.current.indexOf(toastId);
      if (index > -1) {
        toastQueue.current.splice(index, 1);
      }
    }, duration);
  };
  ...
  const handleCedulaChange = (e) => {
    ...
    if (soloNumeros.trim()) {
      const validacion = validarCedulaColombiana(soloNumeros);
  ...
  const handlePlacaChange = (e) => {
    const valor = e.target.value;
    // Limitar a máximo 6 caracteres
    const placaLimitada = valor.slice(0, 6);
    setPlaca(placaLimitada);
    
    // Validar en tiempo real si hay contenido
    if (placaLimitada.trim()) {
      const validacion = validarPlacaColombiana(placaLimitada);
  ...
  const handleConsultar = async () => {
    if (!cedula.trim() || !placa.trim()) {
      showLimitedToast('Por favor complete ambos campos', 'error');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setConsultado(true);
        setConsultaCedula(cedula);  // Guardar valores de la consulta exitosa
        setConsultaPlaca(placa);     // Guardar valores de la consulta exitosa
        showLimitedToast('Consulta realizada exitosamente', 'success');
        ...
        showLimitedToast(data.message || 'Error al consultar la información', 'error');
      }
    } catch (error) {
      console.error('Error al consultar:', error);
      showLimitedToast('Error al consultar la información. Verifique que el backend esté funcionando.', 'error');
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleNuevaConsulta = () => {
    ...
    setCedula('');
    setPlaca('');
    setConsultaCedula('');  // Limpiar valores guardados de consulta
    setConsultaPlaca('');   // Limpiar valores guardados de consulta
    setActiveTab('perfil');
    setErrores({ cedula: '', placa: '' });
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      <header className={`shadow-md p-4 flex justify-between items-center transition-colors duration-300 ${
        ...
        <h1 className="text-2xl font-bold">InfoCar</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 gap-6">
            ...
            <div>
              <input
                ...
                className={`border p-2 rounded transition-colors duration-300 w-full ${
                  errores.cedula 
                    ? isDarkMode
                      ? 'border-red-500 bg-gray-700 text-white placeholder-gray-300 focus:border-red-400 focus:bg-gray-600'
                      : 'border-red-500 bg-white text-gray-800 placeholder-gray-500 focus:border-red-400'
                    : isDarkMode 
                      ? 'bg-gray-700 border-gray-500 text-white placeholder-gray-300 focus:border-blue-400 focus:bg-gray-600' 
            ...
            <section className={`p-4 rounded-2xl border transition-colors duration-300 ${
              ...
              <div className="flex justify-between items-center">
                <div className={`text-sm transition-colors duration-300 ${
                  ...
                  <strong>Consulta realizada:</strong> Cédula {consultaCedula} - Placa {consultaPlaca}
                </div>
                <button 
        ...
        <section className={`p-6 rounded-2xl shadow transition-colors duration-300 ${
          ...
          <h2 className="text-xl font-semibold mb-4">
            {documentosDataDinamico[activeTab]?.nombre || 'Documentos del Vehículo'}
          </h2>
            ...
            <div>
              ...
              <div className="flex flex-col gap-1">
                <label className={`flex items-center transition-colors duration-300 ${
                  ...
                  <input type="checkbox" className={`mr-2 ${isDarkMode ? 'accent-blue-400' : 'accent-blue-500'}`} /> SOAT
                </label>
                <label className={`flex items-center transition-colors duration-300 ${
                  isDarkMode ? 'text-gray-300' : 'text-gray-700'
                }`}>
                  <input type="checkbox" className={`mr-2 ${isDarkMode ? 'accent-blue-400' : 'accent-blue-500'}`} /> Tecnomecánica
                </label>
                <label className={`flex items-center transition-colors duration-300 ${
                  isDarkMode ? 'text-gray-300' : 'text-gray-700'
                }`}>
                  <input type="checkbox" className={`mr-2 ${isDarkMode ? 'accent-blue-400' : 'accent-blue-500'}`} /> Impuesto
                </label>
              </div>
            </div>
            <div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <div className={`text-sm transition-colors duration-300 ${
              ...
              © 2025 InfoCar. Todos los derechos reservados.
            </div>
